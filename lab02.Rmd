...
test<-df.h[,c(1,8:25)]
library(tidyr)
df.h_long <- gather(df.h, faixas, pop, x00_04:x85_mais) %>% mutate(Sex = "homem")
df.m_long <- gather(df.m, faixas, pop, x00_04:x85_mais) %>% mutate(Sex = "mulher")
...
df.test <- filter(df.long, Cod_setor==setor_escolhido)

plotPyramid(df.test, age="faixas", sex="Sex", pop="pop")
